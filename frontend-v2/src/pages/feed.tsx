...
export default function SwapFeed() {
    ...
    // ✅ Real matches state
    const [sidebarMatches, setSidebarMatches] = useState<any[]>([]);
    ...
    // ✅ Fetch real matches for sidebar
    const fetchSidebarMatches = async () => {
        try {
            const token = localStorage.getItem("swapifhy_token");
            if (!token) return;
            const res = await fetch(`${API_URL}/api/match/sync-matrix`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            const data = await res.json();
            if (data.matches) setSidebarMatches(data.matches.slice(0, 4)); // show top 4
        } catch (err) {
            console.error("Sidebar matches failed", err);
        } finally {
    ...
    const handleDelete = async (postId: string) => {
        ...
        try {
            ...
            if (res.ok) { setPosts(posts.filter(p => p.id !== postId)); setActiveDropdownId(null); }
    ...
    const handleLike = async (postId: string, isLiked: boolean) => {
        ...
        try {
            ...
            setPosts(posts.map(p => {
                if (p.id === postId) {
                    const likes = isLiked
                        ? p.likes.filter((l: any) => l.userId !== activeUser.id)
        ...
        <div className="w-full min-h-screen bg-background relative overflow-x-hidden pt-32 pb-24 bg-grid">
            ...
            <div className="max-w-[1080px] mx-auto px-6 relative z-10 w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="glass-elite rounded-2xl overflow-hidden sticky top-32">
                        ...
                        <div className="px-5 pb-6 relative text-center">
                            <div className="space-y-2">
                                <h1 className="text-3xl font-heading font-bold text-foreground tracking-tight leading-tight">
                                    Swap <span className="text-gradient">Feed</span>
                                </h1>
                                <p className="text-muted-foreground font-medium text-sm leading-relaxed">
                                    Discover people in the <span className="text-foreground italic">Swapifhy Hub</span>.
                                </p>
                            </div>
                            ...
                            <h3 className="text-foreground font-bold text-base mt-10 tracking-tight">{activeUser?.name || "..."}</h3>
                ...
                <div className="col-span-1 lg:col-span-6 space-y-8">
                    ...
                    {/* Broadcast Composer */}
                    <div className={`glass-elite rounded-2xl p-5 md:p-6 transition-all duration-300 ${isFocused ? 'ring-1 ring-primary/40' : ''}`}>
                        <h3 className="text-xl font-heading font-black text-foreground tracking-tighter uppercase italic mb-1">
                            Share <span className="text-primary">Skills</span>
                        </h3>
                        <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-[0.3em] opacity-60 mb-4">
                            Broadcast your learning needs to the network
                        </p>
                        <form onSubmit={handleBroadcast}>
                            <div className="flex gap-4">
                                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-primary/20 to-accent/20 border border-primary/30 flex items-center justify-center text-foreground font-bold shrink-0">
                                    ...
                                    {(["UPDATE", "OFFER", "REQUEST"] as const).map(type => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-3.5 py-1.5 rounded-lg text-[11px] font-tech transition-all flex items-center gap-1.5 ${
                                                    isActive
                                                        ? `bg-${typeColor}/15 text-${typeColor} border-b border-${typeColor}/50`
                    ...
                    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
                        {loading ? (
                            ...
                            <motion.div variants={itemVariants} className="text-center py-24 glass-elite rounded-2xl">
                                ...
                                <p className="text-sm font-medium text-muted-foreground">Be the first to share with your network.</p>
                            </motion.div>
                        ) : posts.map(post => {
                                    ...
                                    <div className="p-5 md:p-7 relative z-10">
                                        ...
                                        <div className="flex items-start justify-between mb-5">
                                            <div className="flex items-center gap-3.5">
                                                <div className="w-11 h-11 rounded-full bg-surface border-[1.5px] border-border flex items-center justify-center text-foreground font-bold cursor-pointer hover:border-primary/50 transition-all">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[12px] text-muted-foreground font-tech tracking-wide">
                                        ...
                                        {editingPostId === post.id ? (
                                            <div className="mb-7 bg-surface/50 rounded-xl border border-primary/30 p-1">
                                                <textarea
                                                    value={editContent}
                                                ...
                                                <div className="flex justify-end gap-2 p-2 border-t border-white/5">
                                                    <button onClick={() => setEditingPostId(null)} className="px-4 py-2 rounded-lg text-xs font-bold text-muted-foreground hover:bg-white/5 transition-all">Cancel</button>
                                                    <button onClick={() => saveEdit(post.id)} className="px-4 py-2 rounded-lg text-xs font-bold bg-primary/20 text-primary hover:bg-primary hover:text-white transition-all">Save Changes</button>
                                                </div>
                                            </div>
                                        ) : (
                                        ...
                                        <div className="flex items-center gap-2 border-t border-border/40 pt-4 mt-2">
                                            <motion.button
                                                ...
                                                className={`flex-1 flex items-center justify-center gap-2.5 py-2 rounded-xl text-[12px] font-bold transition-all ${isLiked ? "text-primary bg-primary/10 border border-primary/20" : "text-muted-foreground border border-transparent hover:bg-surface hover:text-foreground hover:border-border/50"}`}
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`flex-1 flex items-center justify-center gap-2.5 py-2 rounded-xl text-[12px] font-bold transition-all border border-transparent ${expandedComments[post.id] ? "bg-surface text-foreground border-border/50" : "text-muted-foreground hover:bg-surface hover:text-foreground hover:border-border/50"}`}
                                            >
                                            ...
                                            >
                                                <div className="p-5 md:p-7 space-y-5">
                                                    ...
                                                    <div className="flex gap-3.5 mt-6 pt-4 border-t border-border/30">
                                                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/20 to-accent/20 border border-primary/30 shrink-0 flex items-center justify-center text-[11px] font-bold text-foreground mt-0.5">
                                                            {activeUser?.name?.charAt(0)}
                                                        </div>
                                                        <div className="flex-1 relative flex items-center">
                                                            <input
                                                                ...
                                                                className="w-full bg-background border border-border/80 rounded-xl pl-4 pr-12 py-2.5 text-[13px] text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all font-sans"
                                                            />
                                                            <button
                ...
                {/* ── RIGHT: Real Matches Sidebar ── */}
                <div className="hidden lg:block lg:col-span-3 space-y-6">
                    <div className="glass-elite rounded-2xl p-6 sticky top-32">
                        <div className="flex items-center gap-2.5 mb-6">
                            <TrendingUp className="w-4 h-4 text-primary" />
                            <h4 className="font-tech text-xs tracking-widest text-foreground uppercase">
                                Your Top Matches
                            </h4>
                        </div>
                        ...
                        <div className="space-y-4">
                            {matchesLoading ? (
                                ...
                                [...Array(3)].map((_, i) => (
                                    <div key={i} className="flex items-center gap-3 animate-pulse p-2">
                                        ...
                                        <div className="flex-1 space-y-2">
                                            <div className="h-2.5 w-3/4 bg-border/30 rounded" />
                                ...
                                <div className="text-center py-6 opacity-40">
                                    <p className="text-[11px] font-tech text-muted-foreground uppercase tracking-widest">
                                        No matches yet. Add skills in Settings.
                                    </p>
                                </div>
                            ) : (
                                sidebarMatches.map((match, i) => (
                                    <motion.div
                                        ...
                                        className="flex items-center gap-3 group cursor-pointer p-2.5 rounded-xl hover:bg-surface border border-transparent hover:border-border/50 transition-all"
                                        onClick={() => handleInitiateSync(match.id)}
                                    >
                                        {/* Avatar */}
                                        <div className="w-10 h-10 rounded-full bg-primary/10 border border-primary/20 shrink-0 flex items-center justify-center text-primary font-bold text-sm overflow-hidden group-hover:border-primary/50 transition-all">
                                            {match.avatarUrl ? (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[11px] text-muted-foreground truncate">
                                                {match.teaching?.slice(0, 2).join(", ") || "No skills listed"}
                                            </p>
                                        </div>

                                        {/* Match score badge */}
                                        {match.matchScore > 0 && (
                                            <div className="shrink-0 px-2 py-0.5 rounded-md bg-primary/10 border border-primary/20 text-primary text-[9px] font-black uppercase tracking-widest">
                        ...
                        {/* ✅ View More → navigates to /matches */}
                        <button
                            onClick={() => router.push("/matches")}
                            className="w-full mt-6 py-2.5 rounded-xl bg-surface border border-border/80 hover:bg-border/50 hover:border-primary/30 hover:text-primary text-[12px] font-bold tracking-wide text-foreground transition-all"
                        >
                            View More Matches →
                        </button>
                    </div>
