            ...
            <motion.div initial={{ opacity: 0, scale: 0.99 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-7xl mx-auto px-6 pb-40 relative z-10 pt-24">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-32 relative z-10">
                    <div className="flex-1">
                        ...
                        <h1 className="text-4xl md:text-6xl font-heading font-black tracking-tighter text-foreground mb-6 uppercase italic leading-none">
                            MY <span className="text-gradient-elite font-tech">PROFILE</span>
                        </h1>
                        <p className="text-muted-foreground text-sm max-w-xl font-medium tracking-tight leading-relaxed uppercase opacity-80">
                    ...
                    <div className="flex gap-6">
                        {[
                            ...
                            <div key={i} className="px-10 py-8 rounded-[2.5rem] glass-card border-border flex flex-col items-center justify-center min-w-[160px] group hover:border-primary/20 transition-all">
                                ...
                                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.4em] mt-3 opacity-60">{s.label === "Stability" ? "SYNC RELIABILITY" : "GLOBAL STANDING"}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-12">
                    ...
                    <div className="p-10 md:p-14 rounded-[3rem] glass-card border-border relative overflow-hidden shadow-2xl bg-gradient-to-br from-primary/5 to-transparent">
                        <div className="absolute top-0 right-0 p-10 opacity-[0.03] rotate-12"><Zap className="w-32 h-32 text-foreground" /></div>
                        ...
                        <div className="space-y-6 relative z-10">
                            {conversations.length > 0 ? (
                                conversations.slice(0, 3).map((conv, idx) => (
                                    <div key={idx} className="flex items-center gap-6 p-6 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 hover:border-primary/20 transition-all group cursor-pointer" onClick={() => router.push(`/matches`)}>
                                        <div className="w-16 h-16 rounded-2xl bg-surface border border-border overflow-hidden shrink-0">
                                            {conv.partnerAvatar ? <img src={conv.partnerAvatar} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-black text-xs text-foreground">{conv.partnerName.charAt(0)}</div>}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="text-[8px] font-medium text-muted-foreground/60">{new Date(conv.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                            </div>
                                        </div>
                                        <button className="w-12 h-12 rounded-full border border-black/10 dark:border-white/10 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all">
                                ...
                                <div className="text-center py-10 opacity-40">
                                    <p className="text-[10px] font-black text-foreground uppercase tracking-[0.4em]">No Active Sync Requests Detected</p>
                                </div>
                            )}
                        ...
                        <div className="space-y-12 relative z-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    ...
                                    <input value={name} onChange={e => setName(e.target.value)} placeholder="Full Name" className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-8 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-primary/50 transition-all font-tech shadow-inner" />
                                </div>
                                <div className="space-y-4">
                                    <label className="block text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] ml-2">Privacy</label>
                                    <div className="flex gap-2 p-1 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl">
                                        {["PUBLIC", "FOLLOWERS_ONLY", "PRIVATE"].map((tier) => (
                                            <button 
                                                key={tier}
                                                onClick={() => setPrivacy(tier)}
                                                className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all ${privacy === tier ? "bg-primary text-white" : "text-muted-foreground hover:text-foreground"}`}
                                            >
                                                {tier.replace("_", " ")}
                                            </button>
                                        ))}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-4">
                                    ...
                                    <textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="Summarize your professional essence..." rows={4} className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-[2rem] py-6 px-8 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-primary/50 transition-all resize-none font-medium text-base shadow-inner" />
                                </div>
                                <div className="space-y-4">
                                    <label className="block text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] ml-2">Hobbies & Interests</label>
                                    <textarea value={hobbies} onChange={e => setHobbies(e.target.value)} placeholder="What do you enjoy doing outside of work?" rows={4} className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-[2rem] py-6 px-8 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-secondary/50 transition-all resize-none font-medium text-base shadow-inner" />
                                </div>
                            </div>
                    ...
                    <div className="p-10 md:p-14 rounded-[3rem] glass-card border-border relative overflow-hidden shadow-2xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                            <div className="space-y-4">
                                ...
                                <input value={github} onChange={e => setGithub(e.target.value)} placeholder="github.com/yourhandle" className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-6 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-primary/50 transition-all text-sm shadow-inner" />
                            </div>
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] ml-2">LinkedIn Bridge</label>
                                <input value={linkedin} onChange={e => setLinkedin(e.target.value)} placeholder="linkedin.com/in/handle" className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-6 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-primary/50 transition-all text-sm shadow-inner" />
                            </div>
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] ml-2">Instagram Feed</label>
                                <input value={instagram} onChange={e => setInstagram(e.target.value)} placeholder="@handle" className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-6 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-secondary/50 transition-all text-sm shadow-inner" />
                            </div>
                            <div className="space-y-4">
                                ...
                                <input value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} placeholder="+1 234 567 890" className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl py-4 px-6 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:bg-black/10 dark:focus:bg-white/10 focus:border-foreground/50 transition-all text-sm shadow-inner" />
                            </div>
                        </div>
                        <div className="mt-8 pt-8 border-t border-border flex items-center justify-between relative z-10">
                            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.3em] opacity-60 italic">// Operational Link Bridges Active</p>
                            <button onClick={() => handleSave()} disabled={saving} className="px-10 py-4 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl text-[10px] font-black text-foreground uppercase tracking-[0.4em] hover:bg-black/10 dark:hover:bg-white/10 hover:border-secondary/40 transition-all active:scale-95 disabled:opacity-50">
                                {saving ? "SAVING..." : "Save"}
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="p-10 rounded-[3rem] glass-card border-border flex flex-col justify-center gap-4">
                             <div className="flex justify-between items-end mb-4">
                                <div>
                                    ...
                                    <span className="text-3xl font-black text-foreground">2.4k</span>
                                </div>
                        ...
                        <div className="p-10 rounded-[3rem] glass-card border-border flex flex-col justify-center gap-4">
                             <div className="flex justify-between items-end mb-4">
                                <div>
                                    ...
                                    <span className="text-3xl font-black text-foreground">A+</span>
                                </div>
                ...
                <div className="space-y-12">
                    <div className="p-12 rounded-[3.5rem] glass-card border-border relative overflow-hidden flex flex-col items-center text-center shadow-2xl">
                        <div className="w-36 h-36 rounded-[2.5rem] bg-gradient-to-br from-primary/10 to-secondary/10 border border-black/10 dark:border-white/10 shadow-inner flex items-center justify-center mb-10 relative overflow-hidden group cursor-pointer" onClick={() => setShowAvatarAlt(!showAvatarAlt)}>
                            {avatarUrl ? (
                        ...
                        {showAvatarAlt && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="absolute inset-0 bg-[#0a0a0a]/95 backdrop-blur-xl z-50 p-10 flex flex-col items-center justify-center">
                                ...
                                <input value={avatarUrl} onChange={e => setAvatarUrl(e.target.value)} placeholder="Or paste image URL..." className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-white placeholder:text-white/40 text-[10px] focus:outline-none focus:border-primary/50 transition-all mb-6" />
                                
                                <input type="file" ref={fileInputRef} onChange={handleImageUpload} className="hidden" accept="image/*" />
                                <button 
                                    onClick={() => fileInputRef.current?.click()}
                                    disabled={uploading}
                                    className="w-full py-4 rounded-xl bg-primary text-white font-black text-[10px] uppercase tracking-[0.3em] hover:scale-[1.02] active:scale-95 transition-all mb-8 shadow-xl shadow-primary/20"
                                >
                        ...
                        <h3 className="text-3xl font-heading font-black mb-3 text-foreground tracking-tighter uppercase leading-none">{name.split(" ")[0] || "USER"}</h3>
                        <p className="text-[11px] font-black text-primary uppercase tracking-[0.4em] mb-10 italic">Core Architect</p>
                        
                        <div className="w-full space-y-6 pt-10 border-t border-border">
                            <div className="flex justify-between items-center px-4">
                                <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.3em]">Knowledge IQ</span>
                                <span className="text-foreground font-black text-xl italic leading-none">142.5</span>
                            </div>
                            <div className="h-2 w-full bg-black/5 dark:bg-white/5 rounded-full p-0.5"><div className="h-full bg-primary rounded-full w-[85%] shadow-[0_0_15px_rgba(99,102,241,0.6)]" /></div>
                        </div>
                    </div>

                    {/* Network Health Stats */}
                    <div className="p-12 rounded-[3rem] glass-card border-border flex flex-col gap-8 shadow-2xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-12 opacity-[0.03]"><BookOpen className="w-40 h-40 text-foreground" /></div>
