...
import { X, MessageSquare, User, Zap, Search, Clock } from "lucide-react";
                ...
                >
                    ...
                    <div className="px-6 py-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                        <div className="flex items-center gap-3">
                            ...
                            <h4 className="text-[15px] font-black text-white uppercase tracking-widest font-heading">Swap Hub</h4>
                        </div>
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-1">
                                            <h5 className="text-[14px] font-bold text-white group-hover:text-primary transition-colors truncate">{conv.partnerName}</h5>
                                            <span className="text-[9px] font-bold text-zinc-600 uppercase tracking-tighter flex items-center gap-1.5 shrink-0">
                                                <Clock className="w-2.5 h-2.5" />
                                                {new Date(conv.updatedAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}
                                            </span>
                                        </div>
                                        <p className="text-[12px] text-zinc-500 truncate group-hover:text-zinc-400 transition-colors">{cleanJargon(conv.lastMessage)}</p>
                                    </div>
                                </button>
                            ))
                        )}
