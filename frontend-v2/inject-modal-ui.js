...
const newEndOfFile = modalUI + `\n` + endOfFile;

if (code.includes(endOfFile) && !code.includes('Maintenance Modal')) {
    code = code.replace(endOfFile, newEndOfFile);
    fs.writeFileSync('src/pages/admin/index.tsx', code);
    console.log('Successfully injected Maintenance Modal UI!');
} else if (code.includes('Maintenance Modal')) {
